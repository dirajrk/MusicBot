...
class MusicBot(discord.Client):
    ...
    async def cmd_sendch(self, args, leftover_args):
        """
        Usage:
            {command_prefix}sendch <message>
        Sends a message to the channel the bot is on
        """
        if leftover_args:
            args = ' '.join([args, *leftover_args])
        
            await self.safe_send_message(channel, args)
